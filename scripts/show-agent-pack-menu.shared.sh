...
if command -v pwsh >/dev/null 2>&1; then
  ...
  exec pwsh -NoProfile -ExecutionPolicy Bypass -File "$SCRIPT_DIR/show-agent-pack-menu.ps1" "${args[@]}"
fi

printf '# Agent Pack Menu\n\n'
printf 'PowerShell is required for full menu generation on this platform.\n'
