...
status, automatically classified local/LAN Ollama connection, installed-model discovery, per-capability model choice, bounded chat, writing, summarization, verified idle/lifecycle model cleanup, and unsigned PyInstaller one-folder development packaging for Windows, Linux, and macOS. Milestone 22B retains optional Tauri packaging, broader capability composition, activated updates, signed distribution, and remaining native platform gates. Milestone 23 owns native
