...
Today, the maintained engineering surfaces are Continue, Aider, and OpenCode. The runnable local web application begins with a keyboard-accessible security-first provider wizard, then uses a chat-first layout with pinned navigation, compact provider/system setup, sanitized system status, automatically classified local/LAN Ollama connection, evidence-gated automatic model choices for chat/writing/summarization, advanced manual overrides, typed no-file-written results, read-only provider-health/evidence/update status, and bounded idle/lifecycle cleanup on Windows, Linux, and macOS. Unknown installed models remain unverified, missing recommendations never trigger an automatic download, Software remains visibly not admitted, and Images remains visibly provider-profile-required. Local image generation has a validated Linux ComfyUI/SDXL path but is not yet admitted to the web UI. Native desktop image profiles remain evidence-gated, and music/video candidates remain documentation-only.
