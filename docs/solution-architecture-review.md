...
| 17: Agent Surface Compatibility Validation | Partial | Complete for Cline, Aider, and OpenCode, partial for active tracked surfaces | Cline, Aider, and OpenCode meet their current generated-sample evidence gates; Kilo Code and OpenHands do not yet have full live validation evidence. Roo Code is historical only because its upstream project is retired. |
...
| 19: Installer Profiles, Evidence Catalog, And Release Packaging | Partial | Complete for Continue, Aider, and OpenCode, partial for cross-agent parity | Capability Evidence Contract v2, Continue profiles, supported Aider and OpenCode install/configure/health adapters, and packaging are complete; install/configure/test parity remains blocked for Cline, Kilo Code, and OpenHands. |
...
- Resolution of Kilo Code's current local-model task-execution failure with the documented project-local `.kilo/kilo.jsonc` adapter and isolated user profile, followed by write-smoke and scoped-edit validation; and evaluation of a maintained Roo Code successor.
