...
| 22: Unified Product UI And Task Composition | In progress; 22A runnable | Local web chat MVP admitted; broader UI and native packaging gated | System status, session-only Ollama discovery, installed-model selection, bounded chat, cross-origin defenses, and verified unload now run on one cross-platform standard-library implementation. Software, images, composition, persistence, updates, remote access, and optional Tauri packaging remain open. |
