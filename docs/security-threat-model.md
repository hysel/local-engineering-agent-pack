...
- A completed, failed, or cancelled chat leaves a model resident and consumes accelerator power after Haven 42 stops using it.
...
The admitted local-web MVP binds only to `127.0.0.1`; validates the exact Host and Origin; requires a random in-memory request token; rejects cross-site fetch metadata; serves bundled assets under a restrictive Content Security Policy; accepts only bounded JSON; and permits only IP-literal loopback or explicitly selected private-LAN Ollama endpoints through the shared no-redirect provider policy. It exposes no repository, filesystem, shell, process, download, update, or arbitrary provider surface. Chat sends `keep_alive: 0`, explicitly unloads the selected model, verifies the process list, repeats cleanup after provider failure, and retries cleanup during shutdown.


## Residual risk and promotion gates

The local-web admission applies only to status, Ollama discovery, model selection, and `general.chat`; it does not admit the broader renderer, workflow, image, persistence, update, remote-access, or native-package surfaces. No optional desktop runtime ships until actual Windows, Linux, and macOS binaries pass renderer, IPC, canonical-path, lifecycle, update, rollback, packaging, uninstall, privilege, and security tests. Unsupported or failed provider cells remain documentation-only and leave no executable integration.
