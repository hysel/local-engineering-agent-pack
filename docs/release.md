...
Milestone 19 is complete for the promoted supported-surface set. Continue, Aider, and OpenCode have evidence-backed install/configure/test positions and deterministic cross-platform contracts. Failed integrations are excluded from the active catalogs, Roo Code is historical, and OpenHands remains a candidate under a defined but unimplemented isolation boundary. Real-project approved write for non-Continue surfaces remains separately evidence-gated.
