...
Milestone 19 is complete for the promoted supported-surface set. Continue, Aider, and OpenCode have evidence-backed install/configure/test positions and deterministic cross-platform contracts. Cline CLI 3.0.46 and Kilo CLI 7.4.11 are quarantined, Roo Code is historical, and OpenHands remains a candidate under a defined but unimplemented isolation boundary; these surfaces remain visible without counting as supported parity. Real-project approved write for non-Continue surfaces remains separately evidence-gated.
