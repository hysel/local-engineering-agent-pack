...
| Cline | VS Code-compatible editor agent plus CLI | Read-only validated and disposable write-smoke validated; a Devstral realistic scoped-edit attempt passed exact scope and behavior but failed whitespace validation due to mixed line endings | Read/write validation guide, CLI automation guide, and sanitized evidence available | Scoped-edit and real-project approved-write blocked | Use `docs/cline-readonly-validation.md`, `docs/cline-cli-model-testing.md`, and `examples/cline-readonly-validation.md`; repeat the realistic edit with clean whitespace and line-ending verification before promotion. |
...
| Kilo Code | VS Code-compatible editor agent | Configuration scaffolded; live generated-sample validation must be rerun | The unified adapter generates project-local `.kilo/kilo.jsonc`, keeps it out of Git, and provides an npm install plan; the shared CLI wrapper uses an isolated user profile so Kilo reads project configuration predictably | Blocked | Earlier diagnostics used the obsolete `.json` path. Rerun generated-sample read, write, and scoped-edit validation with the documented `.jsonc` configuration before making model or surface claims. |
