...
| Cline | VS Code-compatible editor agent plus CLI | Read-only validated and disposable write-smoke validated; Devstral and Qwen Windows CLI source-edit diagnostics both failed LF/whitespace gates on Cline CLI 3.0.46 | Read/write validation guide, hardened CLI automation, and sanitized evidence available | Windows scoped-edit and real-project approved-write blocked | Use `docs/cline-readonly-validation.md`, `docs/cline-cli-model-testing.md`, and `examples/cline-readonly-validation.md`; retest after a relevant Cline editing fix or upgrade rather than cycling more models on 3.0.46. |
...
| Kilo Code | VS Code-compatible editor agent | CLI 7.4.11 and `.kilo/kilo.jsonc` configuration validated; Devstral read-only passed, but Devstral and Qwen 35B write/scoped gates failed despite exit `0` | The unified adapter generates project-local config and the shared harness uses isolated profiles, external verification, nonzero failure status, and model unload | Write, scoped-edit, and real-project approval blocked | See `examples/kilo-validation.md`; retest only after a Kilo tool-protocol/task-execution fix or version change. |
