...
The local-web application holds its provider endpoint, discovered model names, request token, and current chat, writing, or summarization task in process and browser memory only. Changing modes or starting a new task clears the visible task, and closing the process discards all runtime state. Browser assets and API responses use `Cache-Control: no-store`; the application adds no service worker, local storage, cookies, analytics, or crash upload.
