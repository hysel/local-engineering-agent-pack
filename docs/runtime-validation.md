...
| 2026-07-08 | Generated Java, Go, Rust, SQL, and Infrastructure sample repositories | Continue CLI with ignored local Ollama config and `qwen3.5:9b` | All runtime validation workflows | Partial pass: all five generated samples completed the workflow set without runner aborts; prompt and runner filename-fidelity guardrails were added, but latest pass counts remained 4 to 6 of 12 with `EMPTY_MODEL_OUTPUT` and filename-drift guardrail failures | Keep language packs evidence-gated; filename-fidelity fallback artifacts are now generated for deterministic filename failures; add workflow-specific remediation templates only if non-filename guardrail failures recur. |
| 2026-07-22 | Disposable Linux NVIDIA profile | Ollama 0.32.1 with official Qwen 3.5 9B Q4_K_M and Q8_0 artifacts | Bounded response, performance, GPU residency, structured tool call, engineering patch, cleanup, and rollback | Pass for the exact 4,096-token, concurrency-one cell: both variants passed the functional checks; Q4_K_M used 5.6 GB versus 9.2 GB loaded accelerator memory and generated 79.61 versus 66.50 tokens/s | Retain Q4_K_M for this profile; the disposable Q8_0 artifact was removed. Do not transfer this evidence to another model, runtime, GPU, context, concurrency level, or workload. |
| 2026-07-22 | Disposable Linux NVIDIA RTX 5000 profile | Source-built llama.cpp `b10088`, CUDA 12.4, and revision-pinned Qwen 3.5 9B Q4_K_M | Isolated backend discovery, fixed benchmark, bounded response, structured tool call, engineering patch, cleanup, and service preservation | Pass for the exact 4,096-token, concurrency-one engine cell: 5,285 MiB loaded GPU memory, 63.33 generation tokens/s, valid required tool call, and Git-applicable patch | Admit only this engine-level CUDA profile; do not infer an end-user installer, another NVIDIA architecture, agent-surface writes, or cross-runtime behavior. |
...
Runtime validation now injects filename-fidelity instructions at execution time in addition to prompt-level guidance. The verifier remains the acceptance gate because tested local models can still invent conventional filenames after receiving those instructions. When a workflow fails with `FILENAME_NOT_IN_CONTEXT`, the runner writes a `.filename-fidelity-fallback.md` artifact beside the raw output so reviewers have a safe remediation checklist instead of relying on the failed response.
