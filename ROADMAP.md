...
| Milestone 22: Unified Product UI And Task Composition | In progress; 22A text tools and hardened portable development package runnable | The shared local-web application and unsigned PyInstaller one-folder packages provide system status, inferred local/LAN Ollama scope, model selection, chat, writing, summarization, strict typed warning/failure/recovery envelopes without automatic retry, hostile integrity/archive validation, complete evidence inventories, bounded cleanup, and an effect-free offline update lifecycle simulator. Software, images, composition, persistence, activated updates, signing, public releases, and Tauri remain gated. |
