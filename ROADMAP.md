...
| Milestone 26: Hardware-Adaptive Model Quantization | Two live cells validated | Exact Linux/NVIDIA/Ollama and Windows/AMD/Ollama trusted-artifact comparisons validate Q4_K_M over Q8_0 for their tested profiles; broader platform and workload cells remain open. |
...
- Add provider adapters for general text/chat, writing and summarization, image generation, and the existing engineering workflow dispatcher without assuming one model or provider supports every modality. Done for live-validated local Ollama text and ComfyUI SDXL image adapters plus deterministic engineering route plans that preserve workflow safety levels.
...
6. Add Windows NVIDIA, Windows Intel, Windows AMD, and Apple Silicon paths only when an exact runtime and format have credible native support; keep physical Mac validation last. Windows AMD is done for Ollama 0.32.1, its packaged ROCm 7.1 backend, an RX 7800 XT 16 GB profile, Qwen 3.5 9B Q4_K_M versus Q8_0, a 4,096-token context, and concurrency one. Windows NVIDIA, Windows Intel, and Apple Silicon remain open.
