...
4. Implement first-run navigation and capability availability views over the Milestone 21 registry. The local-web renderer now provides keyboard-accessible setup, five engine-derived capability states, policy disclosures, and read-only health/evidence status; the framework-neutral contracts remain the source of truth and the native renderer remains gated.
...
6. Connect setup, health, model choice, engineering workflows, and evidence views from Milestone 20. The local-web first-run provider wizard, engine-derived text recommendations, typed text results, provider health, evidence boundary, and disabled-update status are done; engineering workflows and broader evidence views remain.
