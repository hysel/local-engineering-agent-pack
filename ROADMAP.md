...
The repository is in early implementation stage. Milestones 1 through 21 are complete for their defined supported-surface, generated-fixture, workflow-foundation, and general-capability scopes. Continue, Aider, and OpenCode form the maintained surface set; failed or retired integrations are removed rather than carried as partial implementations. OpenHands remains a documentation-only candidate with a defined but unimplemented isolation boundary. Milestone 22 is in progress with its Tauri/private-IPC desktop architecture selected plus an agreed first product slice, navigation contract, wireframes, and renderer-safe nonvisual view model, and a read-only security-aware model catalog; runtime implementation remains gated by dependency, security, packaging, and cross-platform evidence. Milestone 23 has a promoted Linux image profile and partial Windows AMD evidence. Milestone 24 has partial Linux CUDA ACE-Step evidence, while Milestone 25 remains documentation-only; neither ships an executable integration. Milestone 26 has its foundation, exact Linux/NVIDIA/Ollama and Windows/AMD/Ollama trusted-artifact comparisons, and a provider/engine/backend abstraction. llama.cpp CUDA passed the exact Linux NVIDIA cell and HIP passed the exact Windows AMD cell while Vulkan failed the patch gate; Intel work is parked pending representative hardware, and conversion, activation, broader workloads, Windows NVIDIA, and Apple Silicon remain open.
...
| Milestone 22: Unified Product UI And Task Composition | In progress | The first-run flow, product-wide progressive onboarding, and nonvisual view model are defined, while 46 engine-side IPC and 55 native-authority policy cases cover the fail-closed boundary. Execution remains disabled; actual native bridge, dependency, and package gates stay open. |
