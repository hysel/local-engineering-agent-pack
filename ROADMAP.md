...
The repository has entered user-visible product implementation. Milestones 1 through 21 are complete for their defined scopes. Milestone 22A now ships a runnable local-web application and security-hardened unsigned PyInstaller one-folder development packaging for sanitized system status, explicit Ollama connection, installed-model discovery, private chat, writing, summarization, and verified unload on Windows, Linux, and macOS. Build inputs are hash-locked and native hosted tests cover hostile resources, shutdown authority, archive structure, checksums, inventories, notices, SBOM, and unsigned provenance. Offline update foundations now also model compatibility, staged/post health, interrupted recovery, rollback, retention, and disabled mode with every machine effect denied. Signing, notarization, installers, activated online updates, production release publication, broader UI composition, and optional Tauri packaging remain separately gated. Milestone 23 has a promoted Linux image profile and partial Windows AMD evidence. Milestone 24 has partial Linux CUDA ACE-Step evidence, while Milestone 25 remains documentation-only. Milestone 26 has its foundation plus exact Linux/NVIDIA and Windows/AMD evidence.
...
| Milestone 22: Unified Product UI And Task Composition | In progress; 22A text tools and hardened portable development package runnable | The shared local-web application and unsigned PyInstaller one-folder packages provide system status, inferred local/LAN Ollama scope, model selection, chat, writing, summarization, hostile integrity/archive validation, complete evidence inventories, bounded cleanup, and an effect-free offline update lifecycle simulator. Software, images, composition, persistence, activated updates, signing, public releases, and Tauri remain gated. |
...
- The admitted local-web slice now exposes five truthful capability states: Chat, Writing, and Summarization require a validated provider; Software is not admitted in the web runtime; Images require a separately promoted provider profile.
...
- Render typed text artifacts and progress/result/error states in the admitted local-web slice without granting filesystem authority. Broader workflow warnings, retry, recovery, and artifact-location UI remain open.
...
6. Connect setup, health, model choice, engineering workflows, and evidence views from Milestone 20. Readiness inspection, zero-effect setup planning, the provider wizard, engine-derived text recommendations, typed text results, provider health, evidence boundary, and disabled-update status are done; real installation, engineering workflows, and broader evidence views remain.
