...
The repository has entered user-visible product implementation. Milestones 1 through 21 are complete for their defined scopes. Milestone 22A now ships a runnable local-web application and unsigned PyInstaller one-folder development packaging for sanitized system status, explicit Ollama connection, installed-model discovery, private chat, writing, summarization, and verified unload on Windows, Linux, and macOS. Signing, notarization, installers, activated online updates, production release publication, broader UI composition, and optional Tauri packaging remain separately gated. Milestone 23 has a promoted Linux image profile and partial Windows AMD evidence. Milestone 24 has partial Linux CUDA ACE-Step evidence, while Milestone 25 remains documentation-only. Milestone 26 has its foundation plus exact Linux/NVIDIA and Windows/AMD evidence.
...
| Milestone 22: Unified Product UI And Task Composition | In progress; 22A text tools and portable development package runnable | The shared local-web application and unsigned PyInstaller one-folder packages provide system status, inferred local/LAN Ollama scope, model selection, chat, writing, summarization, integrity verification, and bounded cleanup. Software, images, composition, persistence, activated updates, signing, public releases, and Tauri remain gated. |
