...
| Milestone 17: Agent Surface Compatibility Validation | Partial | Cline has read-only and disposable write-smoke validation evidence, Aider has generated-sample read-only, write-smoke, and richer scoped-edit evidence, and OpenCode has generated-sample read/write-smoke and constrained scoped-edit evidence with Devstral Small 2 24B. Roo Code is retired upstream and is no longer an active target. Kilo Code now uses its documented project-local `.kilo/kilo.jsonc` configuration and an isolated CLI user profile; with a separate preload stage, Devstral Small 2 24B passes generated-sample read-only validation, while write-smoke and scoped-edit remain pending. Full tracked-surface validation remains incomplete for Kilo Code, OpenHands, and explicitly approved OpenCode real-repository validation. |
