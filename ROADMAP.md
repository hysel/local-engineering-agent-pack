...
| Milestone 17: Agent Surface Compatibility Validation | Partial | Cline has read-only and disposable write-smoke evidence, but Cline CLI 3.0.46 Windows source-edit diagnostics with Devstral and Qwen both failed LF/whitespace gates, so scoped edits remain blocked pending a relevant Cline fix or upgrade. Aider has generated-sample read-only, write-smoke, and richer scoped-edit evidence, and OpenCode has generated-sample read/write-smoke and constrained scoped-edit evidence with Devstral Small 2 24B. Roo Code is retired upstream. Kilo Code remains blocked at write execution, and OpenHands plus explicitly approved OpenCode real-repository validation remain incomplete. |
...
- Kilo Code has a dry-run-capable test harness, native-Ollama scaffolded install/configure adapter, explicit `code` agent selection, and isolated user-profile execution, but renewed live local-model diagnostics remain blocked at task execution. Roo Code is retained only for historical compatibility because its upstream project is retired.
