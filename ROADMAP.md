...
The repository has entered user-visible product implementation. Milestones 1 through 21 are complete for their defined scopes. Milestone 22A now ships a runnable local-web application and security-hardened unsigned PyInstaller one-folder development packaging for sanitized system status, immutable-digest Ollama recommendations, private chat, writing, summarization, provider run metrics, plan-only registered read-only software workflows, bounded effect-free composition planning, the promoted Linux ComfyUI/SDXL image flow, and verified cleanup on Windows, Linux, and macOS. Build inputs are hash-locked and native hosted tests cover hostile resources, shutdown authority, relocation, repeated lifecycle, occupied ports, hostile environments, archive structure, checksums, inventories, notices, SBOM, and unsigned provenance. Offline update foundations also model compatibility, staged/post health, interrupted recovery, rollback, retention, and disabled mode with every machine effect denied. Signing, notarization, installers, activated online updates, production release publication, workflow execution, executable composition, and optional Tauri packaging remain separately gated. Milestone 23 has a promoted Linux image profile and partial Windows AMD evidence. Milestone 24 has partial Linux CUDA ACE-Step evidence, while Milestone 25 remains documentation-only. Milestone 26 has its foundation plus exact Linux/NVIDIA and Windows/AMD evidence.
...
| Milestone 22: Unified Product UI And Task Composition | In progress; local tools and hardened portable development package runnable | The shared local-web application and unsigned PyInstaller one-folder packages provide system status, inferred local/LAN Ollama scope, immutable-digest model selection, provider run metrics, chat, writing, summarization, plan-only read-only software workflows, bounded effect-free composition plans, the promoted loopback ComfyUI/SDXL image flow, strict typed envelopes, hostile integrity/archive validation, complete evidence inventories, bounded cleanup, and an effect-free offline update lifecycle simulator. Workflow execution, executable composition, persistence, activated updates, signing, public releases, and Tauri remain gated. |
...
10. Add bounded multi-step composition with explicit intermediate artifacts and approvals. The registry-backed dependency planner, cancellation-before-execution, metadata-only intermediate references, and hostile tests are done without effects; executable dispatch, effect-specific opaque approvals, runtime retry, and recovery remain gated.
